...
def lag_F(w, b, mu, C, gamma, y_val, z_val, y_train, z_train):
    ...
    loss_lower = (1/2) * ((w**2).sum() + (b**2).sum())

    restr_lower = 1 - C - y_train * F.linear(z_train, w, b).squeeze()

    return 1/gamma*loss_upper + loss_lower + mu @ restr_lower
...
def pbgd_free(x_train, y_train, x_val, y_val, x_test, y_test, hparams, epochs, early_stopping_th = False,verbose=True):
    ...
    for epoch in range(epochs):
        ...
        C_tensor_val.data -= eta*(C_tensor_val.grad.detach()) #eta*(C_tensor_val.grad.detach() + gam*mug - muF)
...
if __name__ == "__main__":
    ...
    for seed in range(10):
        ...
        metrics_seed, variables_seed = pbgd_free(x_train, y_train, x_val, y_val, x_test, y_test, hparams, epochs)
