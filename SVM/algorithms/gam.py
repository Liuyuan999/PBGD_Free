...
def gam(x_train, y_train, x_val, y_val, x_test, y_test, hparams, epochs, early_stopping_th = False, verbose=True):
    ...
    for epoch in range(epochs):
        ...
        metrics.append({
            ...
            'test_acc': test_acc,
            'test_loss': test_loss,
            'loss_lower': 0.5*(torch.linalg.norm(w_tensor.detach())**2 + torch.linalg.norm(b_tensor.detach())**2 ),
            'time_computation': time.time() - algorithm_start_time
        })


        begin=time.time()
        prob.solve(solver='ECOS', abstol=2e-3,reltol=2e-3,max_iters=50, warm_start=True)  
        end=time.time()
