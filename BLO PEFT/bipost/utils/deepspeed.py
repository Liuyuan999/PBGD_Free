...
class DeepspeedStrategy(ABC):
    ...
    def create_optimizer(self, model, model_param=None, **kwargs) -> Optimizer:
        if isinstance(model, Actor):
            model = model.model
        if model_param is not None:
            freeze_model(model,model_param)
        # Optimizer
    ...
    def backward(self, loss: torch.Tensor, model: nn.Module, optimizer: optim.Optimizer, scheduler = None, **kwargs) -> None:
        if isinstance(model, Actor):
            model = model.model
        if scheduler is not None:
            try:
                ...
                print("Warning: fail to set the optimizer")
        model.backward(loss)
    ...
    def save_head(self, model: nn.Module, tokenizer, output_dir, output_name = "head.pt", **kwargs) -> None:
        if self.is_rank_0():
            os.makedirs(output_dir, exist_ok=True)

        # Unwrap for DeepSpeed / DDP
        model_to_save = self._unwrap_model(model)

        # Get the target head module
        output_layer_name = self.args.output_layer_name
        head_module = getattr(model_to_save, output_layer_name)
        ...
        for name, param in head_module.named_parameters():
            ...
            with deepspeed.zero.GatheredParameters(params_to_fetch, enabled=len(params_to_fetch) > 0):
                if self.is_rank_0():
                    output_state_dict[name] = param.data.cpu()

        if self.is_rank_0():
            # Save the head parameters
            torch.save(output_state_dict, os.path.join(output_dir, f"{output_layer_name}_"+output_name))
